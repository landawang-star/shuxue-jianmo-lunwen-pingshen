...
4. **⚠️ 论文类型识别（必做，影响后续所有评分）**：按 `references/国赛评审标准.md` 6.7 节识别论文属于哪类：
   - ☐ **A. 传统建模型**（微分方程/优化规划/图论/概率统计）
   - ☐ **B. 数据分析型**（统计/回归/主成分/聚类，结果以统计图表呈现）
   - ☐ **C. 机器学习型**（算法训练预测为主，结果以准确率/F1/AUC/混淆矩阵呈现，图表偏少是常态）
   
   识别后在报告"基本信息"中标注类型，后续格式、结果、创造性维度的评分须按 6.7 类型化调整执行。
...
- 篇幅：
- 完成日期：
- 论文类型：☐A传统建模型 ☐B数据分析型 ☐C机器学习型（识别依据：____）
- 章节完整度：___/8 个必含章节
...
9. **论文类型化评分纪律**：评分前必须识别论文类型（A/B/C，见 references 6.7）。对机器学习型(C类)论文，**不得因"图表偏少"扣分**（领域特征）；不得因文献偏少扣分；创新识别重点转移到特征工程/模型改进/半监督引入。若格式+文献维度合计扣分使总分被压低，**必须在综合评价中明确标注"扣分是否源于论文类型特征而非真实缺陷"**，提示人工评委注意可能的系统性低估
